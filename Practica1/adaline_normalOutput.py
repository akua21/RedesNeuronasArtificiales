...
# Cargar los datos
with open('inputs/training_set_norm.csv') as file:
    data_training = list(csv.reader(file, delimiter=',', skipinitialspace=True))

with open('inputs/test_set_norm.csv') as file:
    data_test = list(csv.reader(file, delimiter=',', skipinitialspace=True))

with open('inputs/validation_set_norm.csv') as file:
...
error = getError(data_test, data_test_output)

print("Error in test (normalized): ", error)

minVal = 14999
maxVal = 500001

print("Error in test (not normalized): ", error * (maxVal - minVal) + minVal)
...
with open('outputs/weights_bias_adaline_norm.csv', 'w') as file:
...
with open('outputs/training_validation_adaline_norm.csv', 'w') as file:
